...
// ── Claude AI (Anthropic) ────────────────────────────────────────────────────
const SYSTEM_PROMPT = `You are a warm, knowledgeable and friendly assistant for Buffalo-Nek High School in Mount Ayliff, Eastern Cape, South Africa.
...
School details:
- Name: Buffalo-Nek High School (Buffalo-Nek High School)
- Location: Njijini A/A, Emthonjeni Location, Mount Frere, 5090
- District: Alfred Nzo West Education District
- Municipality: Umzimvubu Local Municipality
- Phone: +27 72 417 6535 / (039) 254 8224
- Email: Principal.200500338@ecschools.org.za
- Website: jojohighschool.com
- Facebook: Buffalo-Nek High School
- Motto: "Legacy of Leadership and Learning"
- Principal: M.A. Mdondolo
- School hours: Monday–Friday 07:30–15:30
- Grades: Grade 8 to Grade 12
- Type: Public No-Fee School
- Learners: Growing community
...
School colours: Bold Red (#009999), White, Gold/Yellow (#D4AF37)
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      ...
      role: 'bot',
      createdAt: Date.now(),
      text: "👋 Hello! Let me help you with anything about Buffalo-Nek High School! Whether it's admissions, boarding, fees, results, activities or anything else — just ask and I'll be happy to assist.",
    },
  ]);
  ...
  return (
    <>
      ...
      {open && (
        ...
        >
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-3 bg-[#D4AF37] text-[#009999] shrink-0">
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                <div className="font-bold text-sm leading-tight truncate">Buffalo-Nek Assistant</div>
                    ...
                    {(Object.entries(LANG_LABELS) as [SupportedLang, string][]).map(([code, label]) => (
                      <button
                        ...
                        className={`w-full text-left px-3 py-2 text-xs transition-colors ${
                          currentLang === code
                            ? 'bg-[#009999] text-white font-bold'
          ...
          <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3 bg-gray-50 scroll-smooth">
            {messages.map((m) => (
              ...
              >
                {m.role === 'bot' && (
                  <div className="w-6 h-6 rounded-full bg-[#D4AF37] flex items-center justify-center shrink-0 mb-0.5 text-[#009999]">
                ...
                <div
                  className={`max-w-[80%] rounded-2xl px-3 py-2.5 text-sm leading-relaxed shadow-sm whitespace-pre-wrap ${
                    m.role === 'user'
                      ? 'bg-[#009999] text-white rounded-br-sm'
            ...
            {isTyping && (
              <div className="flex items-end gap-1.5">
                <div className="w-6 h-6 rounded-full bg-[#009999] flex items-center justify-center shrink-0">
                  <Sparkles size={11} className="text-white" />
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 justify-center">
                  {QUICK_QUESTIONS.map((q) => (
                    <button
                      key={q}
                      onClick={() => send(q)}
                      className="text-[11px] px-2.5 py-1 rounded-full bg-white border border-[#009999]/25 text-[#009999] hover:bg-[#990000] hover:text-white transition-colors font-medium shadow-sm"
                    >
                      {q}
                    </button>
                  ))}
          ...
          <div className="p-3 bg-white border-t border-gray-100 shrink-0">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 rounded-xl border border-gray-200 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-[#009999]/20 focus:border-[#009999]/40 transition-all bg-gray-50 placeholder:text-gray-400"
              ...
              <button
                ...
                className="bg-[#009999] hover:bg-[#990000]/90 text-white px-3 py-2 rounded-xl transition-all disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center shrink-0"
